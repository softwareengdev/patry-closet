...
import { Mail, Check, AlertCircle, ArrowRight, Sparkles, RefreshCw } from 'lucide-react';
        ...
        <div className="w-full max-w-md mx-auto text-center">
            ...
            {status === 'waiting' && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
                    ...
                    <div className="mt-8">
                        <Link
                            to="/account"
                            className="inline-flex items-center gap-2 px-6 py-3 bg-black dark:bg-white text-white dark:text-black rounded-xl font-semibold text-sm"
                        >
            ...
            {status === 'success' && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
                    ...
                    <h2 className="text-2xl font-bold mb-2">{t('auth.emailVerified', 'Email Verified!')}</h2>
            ...
            {status === 'error' && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}>
                    ...
                    <h2 className="text-2xl font-bold mb-2">{t('auth.verificationFailed', 'Verification Failed')}</h2>
                    <p className="text-gray-500 dark:text-gray-400 text-sm mb-6">{error}</p>
                    <div className="flex flex-col gap-3">
                        <button
                            onClick={handleResend}
                            disabled={resendCooldown > 0}
                            className="w-full py-3 bg-black dark:bg-white text-white dark:text-black rounded-xl font-semibold text-sm disabled:opacity-50"
                        >
                            {resendCooldown > 0
                                ? `${t('auth.resendIn', 'Resend in')} ${resendCooldown}s`
